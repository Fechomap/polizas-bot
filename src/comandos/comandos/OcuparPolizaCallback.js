...
class OcuparPolizaCallback extends BaseCommand {
    ...
    register() {
        ...
        this.handler.registry.registerCallback(/sendLeyenda:(.+)/, async ctx => {
            try {
                ...
                this.logInfo(`Iniciando envío de leyenda para póliza ${numeroPoliza}`, {
                    chatId,
                    threadId
                });
                ...
                if (!leyenda) {
                    this.logError(
                        `No se encontró leyenda para enviar. chatId=${chatId}, threadId=${threadId}`
                    );
                    return await ctx.reply(
                        '❌ No se encontró la leyenda para enviar. Inténtalo nuevamente.'
                    );
                }
                ...
                try {
                    ...
                    this.logInfo(
                        `Leyenda enviada al grupo: ${targetGroupId}, messageId=${sentMsg.message_id}`
                    );
                    ...
                    if (messageId) {
                        ...
                        this.logInfo(
                            'No se encontró ID del mensaje para editar, enviando mensaje nuevo'
                        );
                    ...
                    await ctx.reply(
                        '❌ No se pudo enviar la leyenda al grupo. Verifica que el bot esté en el grupo.'
                    );
        ...
        this.handler.registry.registerCallback(/registrar_servicio_(.+)/, async ctx => {
            try {
                ...
                try {
                    ...
                    if (!policy) {
                        this.logError(
                            `No se pudo obtener póliza ${numeroPoliza} para reenviar leyenda`
                        );
                        return;
                    }
                    ...
                    if (
                        savedState &&
                        savedState.coordenadas &&
                        savedState.coordenadas.origen &&
                        savedState.coordenadas.destino
                    ) {
                        ...
                        this.logInfo(
                            `✅ Leyenda explosiva reenviada al grupo ${targetGroupId} exitosamente`
                        );
                    } else {
                        this.logError(
                            'No se encontraron coordenadas guardadas para regenerar leyenda'
                        );
                    }
                } catch (leyendaError) {
        ...
        this.handler.registry.registerCallback(/addServiceFromTime:(.+)/, async ctx => {
            try {
                ...
                this.logInfo(`Iniciando flujo de añadir servicio para póliza: ${numeroPoliza}`, {
                    chatId,
                    threadId
                });
                ...
                try {
                    ...
                    if (savedState) {
                        this.logInfo(
                            `Estado recuperado para addServiceFromTime: origen=${savedState.origin}, destino=${savedState.destination}, time=${savedState.time}`
                        );
                    }
                    ...
                    if (
                        this.handler &&
                        typeof this.handler.handleAddServicePolicyNumber === 'function'
                    ) {
                        // Pasar la póliza directamente
                        this.logInfo(
                            `Llamando directamente a handleAddServicePolicyNumber con ${numeroPoliza}`
                        );
                        await this.handler.handleAddServicePolicyNumber(ctx, numeroPoliza);
                    } else {
                        this.logInfo(
                            'No se encontró handler específico, simulando accion:addservice estándar'
                        );
                        // Falback - Simular accion:addservice
                        this.handler.awaitingServicePolicyNumber.set(chatId, true, threadId);
                        await ctx.reply(
                            '🚗 Introduce el número de póliza para añadir el servicio:'
                        );
                    }
                } catch (flowError) {
                    this.logError('Error al iniciar flujo de addservice:', flowError);
                    await ctx.reply(
                        `❌ Error al iniciar el proceso. Intente usando "Añadir Servicio" desde el menú principal con póliza ${numeroPoliza}`
                    );
                }
            } catch (error) {
        ...
        this.handler.registry.registerCallback(/selectDay:(\d+):(.+)/, async ctx => {
            try {
                ...
                this.logInfo(
                    `Recuperada info de servicio: contactTime=${serviceInfo.contactTime}, origen=${serviceInfo.origen}, destino=${serviceInfo.destino}`
                );
                ...
                try {
                    ...
                    if (!notificationManager || !notificationManager.isInitialized) {
                        this.logInfo(
                            'NotificationManager no está inicializado, la notificación será solo visual'
                        );
                    } else {
    ...
    async handlePhoneNumber(ctx, messageText, threadId = null) {
        ...
        if (!threadId && activeFlows.some(flow => flow.threadId && flow.threadId !== threadId)) {
            this.logInfo(
                'Ignorando mensaje sin threadId mientras hay flujos activos en otros hilos'
            );
            return false;
        }
        ...
        if (!regexTel.test(messageText)) {
            ...
            this.awaitingPhoneNumber.delete(chatId, threadId);
            return await ctx.reply(
                '❌ Teléfono inválido (requiere 10 dígitos). Proceso cancelada.'
            );
        }
    ...
    async handleOrigenDestino(ctx, messageText, threadId = null) {
        ...
        try {
            ...
            if (result.error) {
                ...
                await ctx.reply('❌ Formato inválido. 📍indica *ORIGEN - DESTINO*', {
                    parse_mode: 'Markdown'
                });
                return true;
            }
            ...
            if (cachedData) {
                ...
                this.polizaCache.set(chatId, cachedData, threadId);
                this.logInfo(
                    `Caché de póliza actualizada con origen=${origen}, destino=${destino}`
                );
            }
            ...
            if (origenCoords && destinoCoords) {
                ...
                enhancedData = await this.generateEnhancedLegend(
                    policy,
                    origenCoords,
                    destinoCoords,
                    rutaInfo
                ...
                leyenda =
                    `🚗 Pendiente servicio "${policy.aseguradora}"\n` +
            ...
            try {
                ...
                const sentMsg = await ctx.telegram.sendMessage(targetGroupId, leyenda);
                this.logInfo(
                    `Leyenda enviada automáticamente al grupo: ${targetGroupId}, messageId=${sentMsg.message_id}`
                );
                ...
                this.logInfo(
                    'Flujo automático de leyenda completado exitosamente en handleOrigenDestino'
                );
            } catch (sendError) {
                    ...
                    reply_markup: {
                        ...
                        inline_keyboard: [
                            [
                                ...
                                {
                                    text: '❌ Cancelar',
                                    callback_data: `cancelLeyenda:${numeroPoliza}`
                                }
                            ]
                        ]
                    }
    ...
    async parseOrigenDestinoInput(messageText) {
        ...
        if (coordinates) {
            return {
                error:
                    'Para usar coordenadas, proporciona origen y destino separados por " - ".\n' +
    ...
    async handleServiceCompleted(ctx, serviceData) {
        try {
            ...
            if (!cachedInfo || !cachedInfo.numeroPoliza) {
                this.logError(
                    'No se encontró información del servicio para programar notificación'
                );
                return false;
            }
    ...
    async handleContactTime(ctx, messageText, threadId = null) {
        ...
        try {
            ...
            this.logInfo(
                `Info de servicio recuperada: numeroPoliza=${serviceInfo.numeroPoliza}, origen=${serviceInfo.origen}, destino=${serviceInfo.destino}`
            );
