...
class VehicleRegistrationHandler {
   ...
   */
    static async iniciarRegistro(bot, chatId, userId) {
        ...
        'Ejemplo: 3FADP4EJ2FM123456';
            ...
            await bot.telegram.sendMessage(chatId, '❌ Error al iniciar el registro. Intenta nuevamente.');
            return false;
        }
    }
   ...
   */
    static async procesarSerie(bot, chatId, userId, serie, registro) {
        ...
        `👤 Titular: ${busqueda.vehiculo.titularTemporal}\n\n` +
    ...
    static async procesarFotos(bot, msg, userId, registro) {
        ...
        try {
            ...
            if (uploadResult.url) {
                ...
                await bot.telegram.sendMessage(chatId, '❌ Error al subir foto a Cloudflare. Intenta nuevamente.');
            }

        } catch (error) {
            console.error('Error procesando foto:', error);
            await bot.telegram.sendMessage(chatId, '❌ Error al procesar la foto. Intenta nuevamente.');
        }
    ...
    static async finalizarRegistro(bot, chatId, userId, registro) {
        try {
            ...
            if (!resultado.success) {
                await bot.telegram.sendMessage(chatId, `❌ Error al crear vehículo: ${resultado.error}`);
                return false;
            }
        ...
        `👤 Titular temporal: ${vehicle.titularTemporal}\n` +
