...
interface IUserInputResult {
    origen: ICoordinates | null;
    destino: ICoordinates | null;
    rutaInfo: IRouteInfo | null;
    error: string | null;
}

interface IHereMapsResponse {
    routes: {
        sections: {
            summary: {
                length: number;
...
class HereMapsService {
     ...
     */
    parseCoordinates(input: string): ICoordinates | null {
     ...
     */
    async calculateRoute(origen: ICoordinates, destino: ICoordinates): Promise<IRouteInfo> {
        if (!this.apiKey) {
        ...
        try {
            ...
            const data = (await response.json()) as IHereMapsResponse;
     ...
     */
    private calculateHaversineDistance(origen: ICoordinates, destino: ICoordinates): number {
     ...
     */
    async processUserInput(origenInput: string, destinoInput: string): Promise<IUserInputResult> {
        const resultado: IUserInputResult = {
            origen: null,
            destino: null,
            rutaInfo: null,
            error: null
