...
function probarDistribucionDomicilios() {
    ...
    const estadosOrdenados = Object.entries(contadores)
    ...
    console.log('═══════════════════════════════════');
    console.log(`📊 Zona Metropolitana Total: ${zmCdmx + zmEdomex} (${((zmCdmx + zmEdomex) / totalPruebas * 100).toFixed(1)}%)`);
    console.log(`   - CDMX: ${zmCdmx} (${(zmCdmx / totalPruebas * 100).toFixed(1)}%)`);
    console.log(`   - Edomex: ${zmEdomex} (${(zmEdomex / totalPruebas * 100).toFixed(1)}%)`);
    console.log(`📊 Estados circundantes: ${morelos + hidalgo + puebla + queretaro} (${((morelos + hidalgo + puebla + queretaro) / totalPruebas * 100).toFixed(1)}%)`);
    ...
    for (const [estado, esperadoPorcentaje] of Object.entries(esperado)) {
        ...
        console.log(`${status} ${estado}: ${realPorcentaje.toFixed(1)}% (esperado: ${esperadoPorcentaje}%, diff: ${diferencia.toFixed(1)}%)`);
    }
}
