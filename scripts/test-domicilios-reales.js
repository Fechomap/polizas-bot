...
async function probarDomiciliosReales() {
    ...
    for (let i = 0; i < 5; i++) {
        try {
            ...
            if (datos.coordenadas) {
                console.log(`🌍 Coords: ${datos.coordenadas.lat.toFixed(4)}, ${datos.coordenadas.lng.toFixed(4)}`);
            }
    ...
    console.log(`🌍 Con coordenadas reales: ${analisis.conCoordenadas} (${(analisis.conCoordenadas/domicilios.length*100).toFixed(1)}%)`);
    console.log(`🏛️  Estados únicos: ${analisis.estadosUnicos.size}`);
    console.log(`🏙️  Municipios únicos: ${analisis.municipiosUnicos.size}`);
    console.log(`📧 Correos @prueba.com.mx: ${analisis.correosValidos} (${(analisis.correosValidos/domicilios.length*100).toFixed(1)}%)`);
    console.log(`📮 CPs válidos (5 dígitos): ${analisis.cpsValidos} (${(analisis.cpsValidos/domicilios.length*100).toFixed(1)}%)`);
