...
const reportsHandlerV2_1 = __importDefault(require("./reportsHandlerV2"));
class ReportsHandler {
    static async handleAction(ctx, action) {
        try {
            switch (action) {
                case 'menu':
                    return await adminMenu_1.default.showReportsMenu(ctx);
                case 'monthly':
            ...
            logger_1.default.error('Error en ReportsHandler:', error);
...
¿Qué período deseas analizar?
            ...
            const keyboard = telegraf_1.Markup.inlineKeyboard([
                [
                    telegraf_1.Markup.button.callback('📅 Mes Actual', 'admin_reports_monthly_current'),
                    telegraf_1.Markup.button.callback('📋 Mes Anterior', 'admin_reports_monthly_previous')
                ],
                [
                    telegraf_1.Markup.button.callback('📊 Seleccionar Mes', 'admin_reports_monthly_select'),
                    telegraf_1.Markup.button.callback('📈 Comparativo 6M', 'admin_reports_monthly_comparative')
                ],
                [telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_reports_menu')]
            ]);
            ...
            logger_1.default.error('Error al mostrar opciones mensuales:', error);
    ...
    static async generateMonthlyReportForPeriod(ctx, startDate, endDate, period) {
        try {
            ...
            const loadingMessage = await ctx.editMessageText('📊 *GENERANDO REPORTE EMPRESARIAL*\n' +
                `📅 Período: ${period}\n\n` +
                '⏳ Extrayendo datos de MongoDB...');
            const reportData = await reportsHandlerV2_1.default.getComprehensiveMonthlyDataV2(startDate, endDate);
            await ctx.editMessageText('📊 *GENERANDO REPORTE EMPRESARIAL*\n' +
                `📅 Período: ${period}\n\n` +
            ...
            await auditLogger_1.AuditLogger.log(ctx, 'monthly_report_generated', {
                module: 'reports',
                metadata: {
                    period,
            ...
            logger_1.default.error('Error al generar reporte mensual:', error);
    ...
    static generateReportSummary(reportData, period) {
        return `
...
📋 Reporte generado el ${new Date().toLocaleString('es-ES')}
    ...
    static async showMonthSelection(ctx, type) {
        try {
...
`;
            ...
            for (let i = 0; i <= currentMonth; i++) {
                ...
                buttons.push([telegraf_1.Markup.button.callback(buttonText, callbackData)]);
            }
            if (currentMonth < 11) {
            ...
            buttons.push([telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_reports_monthly')]);
            ...
            logger_1.default.error('Error al mostrar selección de mes:', error);
    ...
    static async generateComparativeReport(ctx) {
        try {
            ...
            const loadingMessage = await ctx.editMessageText('📈 *GENERANDO REPORTE COMPARATIVO*\n\n' +
...
📋 Reporte generado el ${new Date().toLocaleString('es-ES')}
            ...
            await auditLogger_1.AuditLogger.log(ctx, 'comparative_report_generated', {
                module: 'reports',
                metadata: {
            ...
            logger_1.default.error('Error al generar reporte comparativo:', error);
            await ctx.reply('❌ Error al generar el reporte comparativo.');
        }
    }
    static async generateCurrentWeekReport(ctx) {
        try {
            ...
            const reportData = await reportsHandlerV2_1.default.getComprehensiveMonthlyDataV2(startOfWeek, endOfWeek);
            const summaryText = `
...
📋 Reporte generado el ${new Date().toLocaleString('es-ES')}
            ...
            await auditLogger_1.AuditLogger.log(ctx, 'weekly_report_generated', {
                module: 'reports',
                metadata: {
                    period: 'current_week',
            ...
            logger_1.default.error('Error al generar reporte semanal:', error);
            await ctx.reply('❌ Error al generar el reporte semanal.');
        }
    }
    static async generateLastWeekReport(ctx) {
        try {
            ...
            const reportData = await reportsHandlerV2_1.default.getComprehensiveMonthlyDataV2(startOfLastWeek, endOfLastWeek);
            const summaryText = `
...
📋 Reporte generado el ${new Date().toLocaleString('es-ES')}
            ...
            await auditLogger_1.AuditLogger.log(ctx, 'last_week_report_generated', {
                module: 'reports',
                metadata: {
                    period: 'last_week',
            ...
            logger_1.default.error('Error al generar reporte de semana pasada:', error);
            await ctx.reply('❌ Error al generar el reporte de semana pasada.');
        }
    }
    static async generateWeeklyComparison(ctx) {
        try {
            ...
            const [currentWeekData, lastWeekData] = await Promise.all([
                reportsHandlerV2_1.default.getComprehensiveMonthlyDataV2(startOfWeek, endOfWeek),
                reportsHandlerV2_1.default.getComprehensiveMonthlyDataV2(startOfLastWeek, endOfLastWeek)
            ]);
...
📋 Reporte generado el ${new Date().toLocaleString('es-ES')}
            ...
            await auditLogger_1.AuditLogger.log(ctx, 'weekly_comparison_generated', {
                module: 'reports',
                metadata: {
            ...
            logger_1.default.error('Error al generar comparación semanal:', error);
...
📋 Reporte ejecutivo generado el ${new Date().toLocaleString('es-ES')}
            ...
            await auditLogger_1.AuditLogger.log(ctx, 'executive_report_generated', {
                module: 'reports',
                metadata: {
                    period,
                    startDate: startDate.toISOString(),
            ...
            logger_1.default.error('Error al generar reporte ejecutivo:', error);
...
¿Qué reporte deseas generar?
            `.trim();
            const keyboard = telegraf_1.Markup.inlineKeyboard([
                [telegraf_1.Markup.button.callback('📅 Semana Actual', 'admin_reports_weekly_current')],
                [telegraf_1.Markup.button.callback('📋 Semana Pasada', 'admin_reports_weekly_previous')],
                [telegraf_1.Markup.button.callback('📈 Comparación', 'admin_reports_weekly_comparison')],
                [telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_reports_menu')]
            ]);
            await ctx.editMessageText(menuText, {
                parse_mode: 'Markdown',
            ...
            logger_1.default.error('Error al mostrar reportes semanales:', error);
...
¿Qué período deseas analizar?
            `.trim();
            const keyboard = telegraf_1.Markup.inlineKeyboard([
                [telegraf_1.Markup.button.callback('📅 Mes Actual', 'admin_reports_executive_month')],
                [telegraf_1.Markup.button.callback('📋 Trimestre', 'admin_reports_executive_quarter')],
                [telegraf_1.Markup.button.callback('📈 Semestre', 'admin_reports_executive_semester')],
                [telegraf_1.Markup.button.callback('🏆 Año Completo', 'admin_reports_executive_year')],
                [telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_reports_menu')]
            ]);
            await ctx.editMessageText(menuText, {
                parse_mode: 'Markdown',
            ...
            logger_1.default.error('Error al mostrar opciones ejecutivas:', error);
