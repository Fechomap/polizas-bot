...
describe('Flujo Completo de Registro de Vehículos - Test de Fotos', () => {
    ...
    describe('Flujo completo: Datos + Fotos obligatorias + Finalizar', () => {
        test('debe completar el flujo completo con fotos obligatorias', async () => {
            ...
            const inicioOk = await VehicleRegistrationHandler.iniciarRegistro(
                mockBot,
                chatId,
                userId
            );
            ...
            await VehicleRegistrationHandler.procesarMensaje(
                mockBot,
                { chat: { id: chatId }, text: '1HGBH41JXMN109186' },
                userId
            );
            await VehicleRegistrationHandler.procesarMensaje(
                mockBot,
                { chat: { id: chatId }, text: 'Toyota' },
                userId
            );
            await VehicleRegistrationHandler.procesarMensaje(
                mockBot,
                { chat: { id: chatId }, text: 'Corolla' },
                userId
            );
            await VehicleRegistrationHandler.procesarMensaje(
                mockBot,
                { chat: { id: chatId }, text: '2023' },
                userId
            );
            await VehicleRegistrationHandler.procesarMensaje(
                mockBot,
                { chat: { id: chatId }, text: 'Blanco' },
                userId
            );
            ...
            jest.clearAllMocks();
            const finalizarSinFotos = await VehicleRegistrationHandler.finalizarRegistro(
                mockBot,
                chatId,
                userId,
                registro
            ...
            const msgFoto1 = {
                chat: { id: chatId },
                photo: [{ file_id: 'photo1_small' }, { file_id: 'photo1_large' }]
            };
            ...
            jest.clearAllMocks();
            const finalizarConFotos = await VehicleRegistrationHandler.finalizarRegistro(
                mockBot,
                chatId,
                userId,
                registroConDosFotos
        ...
        test('debe rechazar finalización sin fotos mediante callback', async () => {
            ...
            const {
                vehiculosEnProceso: vehiculosMap
            } = require('../../src/comandos/comandos/VehicleRegistrationHandler');
