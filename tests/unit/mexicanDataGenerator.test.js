...
describe('MexicanDataGenerator', () => {
  ...
  describe('generarNombreMexicano', () => {
    test('debe generar un nombre mexicano válido', () => {
      ...
      expect(nombre.nombreCompleto).toBe(`${nombre.nombre} ${nombre.apellido1} ${nombre.apellido2}`);
    });
  ...
  describe('generarTelefonoMexicano', () => {
    test('debe generar teléfono mexicano válido', () => {
      ...
      expect(telefono).toMatch(/^\+52 \d{2,3} \d{4} \d{3,4}$/);
      expect(telefono.startsWith('+52')).toBe(true);
    });

    test('debe generar múltiples teléfonos diferentes', () => {
  ...
  describe('generarDireccionMexicana', () => {
    ...
    test('debe incluir estados mexicanos válidos', () => {
      const estadosValidos = [
        ...
        'Puebla', 'Veracruz', 'Guanajuato', 'Chihuahua', 'Sonora', 'Michoacán'
      ];
  ...
  describe('generarDatosMexicanosCompletos', () => {
    test('debe generar datos completos de persona mexicana', () => {
      ...
      expect(datos.telefono).toMatch(/^\+52/);
