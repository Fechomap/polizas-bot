const { createMockModel, mockPolicyData, createMongoError, DUPLICATE_KEY_ERROR } = require('../../mocks/database');
...
describe('PolicyController - Tests básicos funcionando', () => {
    ...
    describe('savePolicy', () => {
        ...
        test('debe manejar error de duplicado', async () => {
            ...
            const duplicateError = createMongoError(
                DUPLICATE_KEY_ERROR, 
                'Duplicate key error', 
                { numeroPoliza: 1 }
            );
            ...
            await expect(savePolicy(policyData)).rejects.toThrow('Ya existe una póliza con el número: POL-001');
        });
    });
