...
describe('BaseAutosCommand', () => {
  ...
  describe('métodos básicos', () => {
    ...
    test('getDescription debe retornar la descripción correcta', () => {
      expect(command.getDescription()).toBe('Base de Datos de Autos - Registro y Asignación de Pólizas');
    });
  });
  ...
  describe('register', () => {
    ...
    test('debe registrar todos los handlers de acciones', () => {
      ...
      expect(mockBot.action).toHaveBeenCalledWith('base_autos:registrar', expect.any(Function));
      expect(mockBot.action).toHaveBeenCalledWith('base_autos:asegurar', expect.any(Function));
      ...
      expect(mockBot.action).toHaveBeenCalledWith(/^vehiculos_pag_(\d+)$/, expect.any(Function));
    });
    ...
    describe('accion:base_autos', () => {
      ...
      beforeEach(() => {
        ...
        const actionCall = mockBot.action.mock.calls.find(call => call[0] === 'accion:base_autos');
      ...
      test('debe manejar errores correctamente', async () => {
        ...
        expect(mockCtx.reply).toHaveBeenCalledWith('❌ Error al mostrar el menú de Base de Autos.');
      });
    });
    ...
    describe('base_autos:registrar', () => {
      ...
      beforeEach(() => {
        const actionCall = mockBot.action.mock.calls.find(call => call[0] === 'base_autos:registrar');
        actionHandler = actionCall[1];
      ...
      test('debe iniciar registro de vehículo correctamente', async () => {
        ...
        expect(VehicleRegistrationHandler.tieneRegistroEnProceso).toHaveBeenCalledWith('user123');
      ...
      test('debe manejar errores en inicio de registro', async () => {
        VehicleRegistrationHandler.iniciarRegistro.mockRejectedValueOnce(new Error('Error'));
    ...
    describe('base_autos:asegurar', () => {
      ...
      beforeEach(() => {
        const actionCall = mockBot.action.mock.calls.find(call => call[0] === 'base_autos:asegurar');
        actionHandler = actionCall[1];
        
        PolicyAssignmentHandler.tieneAsignacionEnProceso = jest.fn().mockReturnValue(false);
        PolicyAssignmentHandler.mostrarVehiculosDisponibles = jest.fn().mockResolvedValue(true);
      });
      ...
      test('debe mostrar vehículos disponibles correctamente', async () => {
        ...
        expect(mockCtx.answerCbQuery).toHaveBeenCalled();
        expect(mockCtx.deleteMessage).toHaveBeenCalled();
        expect(PolicyAssignmentHandler.tieneAsignacionEnProceso).toHaveBeenCalledWith('user123');
    ...
    describe('vehiculo_cancelar', () => {
      ...
      beforeEach(() => {
        const actionCall = mockBot.action.mock.calls.find(call => call[0] === 'vehiculo_cancelar');
        actionHandler = actionCall[1];
    ...
    describe('vehiculo_finalizar', () => {
      ...
      beforeEach(() => {
        const actionCall = mockBot.action.mock.calls.find(call => call[0] === 'vehiculo_finalizar');
        actionHandler = actionCall[1];
    ...
    describe('poliza_cancelar', () => {
      ...
      beforeEach(() => {
        const actionCall = mockBot.action.mock.calls.find(call => call[0] === 'poliza_cancelar');
        actionHandler = actionCall[1];
    ...
    describe('asignar_vehicleId', () => {
      ...
      beforeEach(() => {
        const actionCall = mockBot.action.mock.calls.find(call => 
          call[0].toString() === '/^asignar_(.+)$/'
        );
        actionHandler = actionCall[1];
      ...
      test('debe manejar errores en inicio de asignación', async () => {
        ...
        expect(mockCtx.reply).toHaveBeenCalledWith('❌ Error al iniciar la asignación de póliza.');
      });
    });
    ...
    describe('vehiculos_pag_N', () => {
      ...
      beforeEach(() => {
        const actionCall = mockBot.action.mock.calls.find(call => 
          call[0].toString() === '/^vehiculos_pag_(\\d+)$/'
        );
        actionHandler = actionCall[1];
      ...
      test('debe manejar errores en paginación', async () => {
        PolicyAssignmentHandler.mostrarVehiculosDisponibles.mockRejectedValueOnce(new Error('Error'));
    ...
    describe('accion:volver_menu', () => {
      ...
      beforeEach(() => {
        const actionCall = mockBot.action.mock.calls.find(call => call[0] === 'accion:volver_menu');
        actionHandler = actionCall[1];
      });
